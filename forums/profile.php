...
// SUBSCRIPTION MODIFICATION
...
else if (isset($_POST['updatePdate'])) {
	$user_id=$id;
	if ($pun_user['g_id'] != PUN_ADMIN && ($pun_user['g_moderator'] != '1' || $pun_user['g_mod_ban_users'] == '0'))
	    message($lang_common['No permission']); //User don't have permission to do that
	$user_gid = $db->query('SELECT * FROM '.$db->prefix.'users WHERE id='.$user_id) or error('Unable to fetch current user', __FILE__, __LINE__, $db->error()); 
	$cur_user = $db->fetch_assoc($user_gid) or error('Unable to fetch current user', __FILE__, __LINE__, $db->error());
	if ($cur_user['group_id'] == 4) { //check if user don't have premium group
		$update_user_gid = "UPDATE users SET group_id='5' WHERE id=$user_id";
		$db->query($update_user_gid) or error('Unable to update user', __FILE__, __LINE__, $db->error()); //add premium group to a user
	}
	confirm_referrer('profile.php');
	$effectiveDate = isset($_POST['form[sub]']);
	echo $effectiveDate;
}

else if (isset($_POST['add30days'])) {
	$user_id=$id;
	if ($pun_user['g_id'] != PUN_ADMIN && ($pun_user['g_moderator'] != '1' || $pun_user['g_mod_ban_users'] == '0'))
	    message($lang_common['No permission']); //User don't have permission to do that
	...
	if ($cur_user['group_id'] == 4) { //check if user don't have premium group
		$update_user_gid = "UPDATE users SET group_id='5' WHERE id=$user_id";
		$db->query($update_user_gid) or error('Unable to update user', __FILE__, __LINE__, $db->error()); //add premium group to a user
	} elseif ($cur_user['group_id'] == 5 || !empty($cur_user['premium_date']) ) {
		$stringDate = $cur_user['premium_date'];
	} else {
    ...
    confirm_referrer('profile.php');
	$effectiveDate = date('Y-m-d H:i:s', strtotime($stringDate. ' + 1 month')); //add * month/s to user account
	

    $sql = "UPDATE `users` SET premium_date='$effectiveDate' WHERE id='$user_id' ";
	$db->query($sql) or error('Unable to update user', __FILE__, __LINE__, $db->error());
	redirect('profile.php?section=admin&amp;id='.$user_id, 'User subscription updated!'); //Subscription added!
} else if (isset($_POST['add90days'])) {
	$user_id=$id;
	if ($pun_user['g_id'] != PUN_ADMIN && ($pun_user['g_moderator'] != '1' || $pun_user['g_mod_ban_users'] == '0'))
	    message($lang_common['No permission']); //User don't have permission to do that
    ...
    $date = new DateTime('now');
	$stringDate = $date->format('Y-m-d H:i:s');
	$effectiveDate = date('Y-m-d H:i:s', strtotime($stringDate. ' + 3 months')); //add * month/s to user account
	

    $sql = "UPDATE `users` SET premium_date='$effectiveDate' WHERE id='$user_id' ";
	$db->query($sql) or error('Unable to update user', __FILE__, __LINE__, $db->error());
	redirect('profile.php?section=admin&amp;id='.$user_id, 'User subscription updated!'); //Subscription added!
} else if (isset($_POST['add1year'])) {
	$user_id=$id;
	if ($pun_user['g_id'] != PUN_ADMIN && ($pun_user['g_moderator'] != '1' || $pun_user['g_mod_ban_users'] == '0'))
	    message($lang_common['No permission']); //User don't have permission to do that
    ...
    $date = new DateTime('now');
	$stringDate = $date->format('Y-m-d H:i:s');
	$effectiveDate = date('Y-m-d H:i:s', strtotime($stringDate. ' + 12 months')); //add * month/s to user account
...
else if (isset($_POST['form_sent']))
{
	...
	switch ($section)
	{
		...
		case 'premium': 
		{
			if ($pun_user['group_id'] == 5 || $pun_user['group_id'] == 1) {
...
?>
<div class="blockform">
		<h2><span><?php echo pun_htmlspecialchars($user['username'])?> - Premium user options</span></h2>
		<div class="box">
				...
				<div class="inform">
					<fieldset>
						...
						<div class="infldset">
							<?php
							$id=$user['id'];
							$result1 = $db->query("SELECT * FROM `invites` WHERE `user_id`='$id' AND `used`='0'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
							if ($db->num_rows($result1)){
								?>	<p>Do you have someone you would like to join us? Send them the <a href="register.php">register</a> link along with one of the following codes.</p> <?php
								foreach($result4478 as $item): ?><p><?php echo $item['code'] ?></p><?php endforeach;
							} else {
				...
				<div class="inform">
					...
					<fieldset>
						...
						<div class="infldset">
							...
							$user_gid = $db->query('SELECT * FROM '.$db->prefix.'users WHERE id='.$user_id) or error('Unable to fetch current user', __FILE__, __LINE__, $db->error()); 
							$cur_user = $db->fetch_assoc($user_gid) or error('Unable to fetch current user', __FILE__, __LINE__, $db->error());
							$premium_date = new DateTime($cur_user['premium_date']);
							$userdate = $premium_date->format('Y-m-d H:i:s');
							if($cur_user['group_id'] == 4) {echo 'User dont have active subscriptions';}else{echo '<strong>Premium (Counter-Strike: Global Offensive)</strong> until '.$userdate;}?></p>
							<p><input class="button" type="submit" name="update" value="Submit"></p>
							<input class="button" type="submit" name="add30days" value="Add 30 days">
							<input class="button" type="submit" name="add90days" value="Add 90 days">
							<input class="button" type="submit" name="add1year" value="Add 1 year">	
						</div>
					</fieldset>
				</div>
					<fieldset>
