...
if ($pun_user['g_id'] == 5 || $pun_user['g_id'] == 1 || $pun_user['g_id'] == 2){
	?>
	...
	<div class="blockform">
		...
		<div class="box">
			<div id="shout">
				...
				<label>
					<span></span>
					<input id="shouttext" type="text" onkeypress="handleKeyPress(event);" maxlength="140">
					<div id="shoutvol" class="fake-link"><i class="fa fa-lg fa-volume-up"></i></div>
                ...
                var input = document.getElementById('shouttext');
...
<?php
	...
	if ($cur_forum['last_post'] != '')
	    $last_post = '<a href="viewtopic.php?pid='.$cur_forum['last_post_id'].'#p'.$cur_forum['last_post_id'].'">'.htmlentities($cur_forum1).'</a> <span class="byuser">'.format_time($cur_forum['last_post']).'</span> <span class="byuser">by '.pun_htmlspecialchars($cur_forum['last_poster']).'</span>';
	...
	if ($cur_forum['last_post'] != '')
	{
		...
		$last_post = str_replace('<span class="byuser">'.$lang_common['by'].' '.pun_htmlspecialchars($cur_forum['last_poster']).'</span>', '<span class="byuser">'.$lang_common['by'].' '.$cur_forum['last_poster'].'</span>', $last_post);
	}
	...
	while ($pun_user_online = $db->fetch_assoc($result))
	{
		if ($pun_user_online['user_id'] > 1)
		{
			if ($pun_user['g_view_users'] == '1')
				...
				$users[] = "\n\t\t\t\t".'<dd><span class="gid'.$pun_user_online['group_id'].'">'.pun_htmlspecialchars($pun_user_online['ident']).'</span>';
